...
namespace CBG.FPSMeshTool
{
    ...
    public class FPSMeshToolWindow : EditorWindow
    {
        ...
        string[] mLeftUpperArmPatterns = { "left.*upper.*arm" };
        private string[] mLeftLowerArmPatterns = { "left.*fore.*arm" };
        private string[] mLeftHandPatterns = { "left.*hand" };

        string[] rightUpperArmPatterns = { " right.*upper.*arm" };
        private string[] mRightLowerArmPatterns = { "right.*fore.*arm" };
        private string[] mRightHandPatterns = { "right.*hand" };


        string[] mLeftUpperLegPatterns = { "left.*upper.*leg" };
        private string[] mLeftLowerLegPatterns = { "left.*lower.*leg" };
        private string[] mLeftFootPatterns = { "left.*foot" };

        string[] rightLegPatterns = { "right.*upper.*leg"  };
        private string[] mRightLowerLeg = { "right.*lower.*leg" };
        private string[] mRightFootPatterns = { "right.*foot" };
        ...
        [SerializeField]
        Transform mLeftUpperArmBone;
        Transform mNewLeftUpperArmBone;
        [SerializeField]
        Transform mLeftLowerArmBone;
        Transform mNewLeftLowerArmBone;
        [SerializeField] private Transform mLeftHandBone;
        private Transform mNewLeftHandBone;
        
        [SerializeField]
        Transform rightUpperArm;
        Transform mNewRightUpperArmBone;
        [SerializeField]
        private Transform rightLowerArm;
        private Transform newRightLowerArm;
        [SerializeField] private Transform mRightHandBone;
        private Transform mNewRightHandBone;
        ...
        [SerializeField]
        Transform mLeftUpperLegBone;
        Transform mNewLeftUpperLegBone;
        [SerializeField] private Transform mLeftLowerLegBone;
        private Transform mNewLeftLowerLegBone;
        [SerializeField] private Transform mLeftFoot;
        private Transform mNewLeftFoot;

        [SerializeField]
        Transform mRightUpperLegBone;
        Transform mNewUpperRightLegBone;
        [SerializeField]
        private Transform mRightLowerLegBone;
        private Transform mNewRightLowerLegBone;
        [SerializeField]
        private Transform mRightFoot;
        private Transform mNewRightFoot;
        ...
        [SerializeField]
        bool includeRightUpperArm = true;
        bool newIncludeRightUpperArm;
        [SerializeField]
        bool includeRightLowerArm = true;
        bool newIncludeRightLowerArm;
        [SerializeField]
        ...
        bool validLeftArmBone { get { return IsValidBone(rootBone, mLeftUpperArmBone); } }
        bool validRightUpperArmBone { get { return IsValidBone(rootBone, rightUpperArm); } }
        bool validRightLowerArmBone { get { return IsValidBone(rootBone, rightLowerArm); } }
        bool validLeftLegBone { get { return IsValidBone(rootBone, mLeftUpperLegBone); } }
        bool validRightLegBone { get { return IsValidBone(rootBone, mRightUpperLegBone); } }
        bool validBones { get { return validRootBone && validHeadBone && validLeftArmBone && validRightUpperArmBone && ((!processLegs) || (validLeftLegBone && validRightLegBone)); } }
        ...
        void OnGUI()
        {
            ...
            if (validObject && validRenderers)
            {
                ...
                mNewLeftUpperArmBone = (Transform)EditorGUILayout.ObjectField("Left Arm", mLeftUpperArmBone, typeof(Transform), true);
                if (!validLeftArmBone) EditorGUILayout.HelpBox(boneMessage, MessageType.None);
                mNewRightUpperArmBone = (Transform)EditorGUILayout.ObjectField("Right Upper Arm", rightUpperArm, typeof(Transform), true);
                if (!validRightUpperArmBone) EditorGUILayout.HelpBox(boneMessage, MessageType.None);
                newRightLowerArm = (Transform)EditorGUILayout.ObjectField("Right Arm", rightLowerArm, typeof(Transform), true);

                if (!validRightLowerArmBone) EditorGUILayout.HelpBox(boneMessage, MessageType.None);
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    if (mNewLeftUpperArmBone != mLeftUpperArmBone)
                    {
                        Undo.RecordObject(this, "Changed left arm bone");
                        mLeftUpperArmBone = mNewLeftUpperArmBone;
                    }
                    if (mNewRightUpperArmBone != rightUpperArm)
                    {
                        Undo.RecordObject(this, "Changed right upper arm bone");
                        rightUpperArm = mNewRightUpperArmBone;
                    }
                    if (newRightLowerArm != rightLowerArm)
                    {
                        Undo.RecordObject(this, "Changed right upper arm bone");
                ...
                if (processLegs)
                {
                    mNewLeftUpperLegBone = (Transform)EditorGUILayout.ObjectField("Left Leg", mLeftUpperLegBone, typeof(Transform), true);
                    if (!validLeftLegBone) EditorGUILayout.HelpBox(boneMessage, MessageType.None);
                    mNewUpperRightLegBone = (Transform)EditorGUILayout.ObjectField("Right Leg", mRightUpperLegBone, typeof(Transform), true);
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    {
                        if (mNewLeftUpperLegBone != mLeftUpperLegBone)
                        {
                            Undo.RecordObject(this, "Changed left leg bone");
                            mLeftUpperLegBone = mNewLeftUpperLegBone;
                        }
                        if (mNewUpperRightLegBone != mRightUpperLegBone)
                        {
                            Undo.RecordObject(this, "Changed right leg bone");
                            mRightUpperLegBone = mNewUpperRightLegBone;
                        }
                    }
                    OnBonesUpdated();
                }
                ...
                if (!includeAll)
                {
                    ...
                    newIncludeRightUpperArm = EditorGUILayout.Toggle("Include RightArm", includeRightUpperArm);
                    newIncludeRightLowerArm = EditorGUILayout.Toggle("Include RightLowerArm", includeRightLowerArm);
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    else
                    {
                        ...
                        if (newIncludeRightUpperArm != includeRightUpperArm) includeRightUpperArm = newIncludeRightUpperArm;
                        if (newIncludeRightLowerArm != includeRightLowerArm)
                            includeRightLowerArm = newIncludeRightLowerArm;
        ...
        void GuessBones(GameObject sourceObject)
        {
            ...
            if (validObject && validRenderers)
            {
                ...
                if (validRootBone)
                {
                    ...
                    mLeftUpperArmBone = GetBoneMatch(rootBone, mLeftUpperArmPatterns);
                    rightUpperArm = GetBoneMatch(rootBone, rightUpperArmPatterns);
                    rightLowerArm = GetBoneMatch(rootBone, mRightLowerArmPatterns);
                    mLeftUpperLegBone = GetBoneMatch(rootBone, mLeftUpperLegPatterns);
                    mRightUpperLegBone = GetBoneMatch(rootBone, rightLegPatterns);
                }
                else
                {
                    headBone = null;
                    mLeftUpperArmBone = null;
                    rightUpperArm = null;
                    rightLowerArm = null;
                    mLeftUpperLegBone = null;
                    mRightUpperLegBone = null;
                }
            }
            else
            {
        ...
        void BuildPreviewMeshes()
        {
            if (canShowPreview)
            {
                ...
                for (int i = 0; i < targetRenderers.Length; i++)
                {
                    if (useRenderer[i])
                    {
                        ConvertToFPSMesh(sourceRenderers[i], targetRenderers[i], headBone, mLeftUpperArmBone, rightUpperArm, processLegs, mLeftUpperLegBone, mRightUpperLegBone);
                    }
                }
        ...
        void ConvertToFPSMesh(SkinnedMeshRenderer sourceRenderer, SkinnedMeshRenderer targetRenderer, Transform head, Transform leftArm, Transform rightArm, bool processLegs, Transform leftLeg, Transform rightLeg)
        {
            ...
            Material lowerArmMat = (Material)Resources.Load("FPSMeshMaterials/LowerArmsMat1");
            ...
            Transform[] rightLowerArmBoms = GetBones(rightLowerArm, bones);
            int[] rightLowerArmBoneIndices = GetBoneIndices(rightLowerArmBoms, bones);
            ...
            Dictionary<int, int[]> boneDict = GetBoneDict(headBoneIndices, leftArmBoneIndices, rightArmBoneIndices, rightLowerArmBoneIndices, leftLegBoneIndices, rightLegBoneIndices);
            ...
            for (int subMeshID = 0; subMeshID < baseMesh.subMeshCount; subMeshID++)
            {
                ...
                List<Triangle> rightLowerArmTriangels = new List<Triangle>();
                ...
                while (index < triangles.Length)
                {
                    ...
                    else if (belongsTo == rightLowerArmBoneIndices)
                    {
                        rightLowerArmTriangels.Add(tri);
                    }
                ...
                List<Triangle> rightUpperArmTris = new List<Triangle>();
                List<Triangle> rightLowerArmTris = new List<Triangle>();
                if (includeLeftArm)
                {
                    rightUpperArmTris.AddRange(leftArmTriangles);
                }
                if (includeRightUpperArm)
                {
                    rightUpperArmTris.AddRange(rightArmTriangles);
                }
                if (includeRightLowerArm)
                {
                    rightLowerArmTris.AddRange(rightLowerArmTriangels);
                }
                if (rightLowerArmTris.Count > 0)
                {
                    AddSubMesh(subMeshes, mats, rightLowerArmTris.ToArray(), lowerArmMat);
                    AddMaterial(materials, materialNames, sourceRenderer.sharedMaterials[subMeshID], "lowerArms");

                }
                if (rightUpperArmTris.Count > 0)
                {
                    AddSubMesh(subMeshes, mats, rightUpperArmTris.ToArray(), armsMat);
        ...
        Dictionary<int, int[]> GetBoneDict(int[] headBoneIndices, int[] leftArmBoneIndices, int[] rightArmBoneIndices, int[] rightLowerArmBonesIndicies, int[] leftLegBoneIndices, int[] rightLegBoneIndices)
        {
            ...
            foreach (int bone in rightLowerArmBonesIndicies)
            {
                boneDict[bone] = rightLowerArmBonesIndicies;
            }
