...
/** 
* @file PassthroughServer.cs
* @brief Contains the PassthroughServer class
...
{
    /// <summary>
    /// PassthroughServer to handle multiple connection stream requests, with persistent connection
    /// </summary>
    public class PassthroughServer: IBrainpackServer
    {
        ...
        private void AcceptCallback(IAsyncResult vAsyncResult)
        {
            ...
            catch (Exception vE)
            {
                if (vStateObject.mClientSocket != null)
                {
                    vStateObject.mClientSocket.Close(); lock (mConnections)
                    {
                        if (mConnections.Contains(vStateObject))
                        {
                            mConnections.Remove(vStateObject);
            ...
            try
            {
                ...
                if (vBytesRead > 0)
                {
                    ...
                    if (vContent.IndexOf(PacketSetting.EndOfPacketDelim) > -1)
                    {
                        ...
                        ServerCommandRouter.Instance.Process(vStateObject, vHPacket);
                    }
